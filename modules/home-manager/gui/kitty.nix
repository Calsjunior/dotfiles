...
}:
{
  ...
  config = lib.mkIf config.gui.kitty.enable (
    let
      packOpt = plugin: "${config.home.homeDirectory}/.local/share/nvim/site/pack/core/opt/${plugin}";
      smartSplitsDir = packOpt "smart-splits.nvim";
      kittyScrollbackDir = packOpt "kitty-scrollback.nvim";
    in
    {
      programs.kitty = {
        enable = true;
          ...
          + (
            if config.cli.neovim.enable then
              ''
                ...
                # Scrollback Pager
                action_alias kitty_scrollback_nvim kitten ${kittyScrollbackDir}/python/kitty_scrollback_nvim.py
      ...
      xdg.configFile."kitty/neighboring_window.py".source = lib.mkIf config.cli.neovim.enable (
        config.lib.file.mkOutOfStoreSymlink "${smartSplitsDir}/kitty/neighboring_window.py"
      );

      xdg.configFile."kitty/relative_resize.py".source = lib.mkIf config.cli.neovim.enable (
        config.lib.file.mkOutOfStoreSymlink "${smartSplitsDir}/kitty/relative_resize.py"
      );
    }
  );
}
